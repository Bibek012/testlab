...
export const ErrorState = ({ error, onRetry, className, compact }: Props) => {
  const isPermissionError = error?.name === 'FirestorePermissionError' || error?.code === 'permission-denied';
  ...
  return (
    <div className={cn(
      ...
      <div className={cn(
        ...
        {isOffline ? <WifiOff className={compact ? "w-5 h-5" : "w-8 h-8"} /> : <AlertCircle className={compact ? "w-5 h-5" : "w-8 h-8"} />}
      </div>

      <div className="space-y-2">
        <h3 className={cn("font-headline font-bold", compact ? "text-sm" : "text-xl")}>
          {isOffline ? "Network Disconnected" : isPermissionError ? "Access Restricted" : "Something went wrong"}
        </h3>
        <p className={cn("text-muted-foreground max-w-xs", compact ? "text-[10px]" : "text-sm")}>
          {isOffline ? "Please check your internet connection and try again." : 
           isPermissionError ? "You don't have required permissions to view this resource." : 
