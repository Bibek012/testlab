...
export const MockTestList = ({ examId, categorySlug, stateSlug }: MockTestListProps) => {
  ...
  return (
    <div className="space-y-8 md:space-y-12 py-6 md:py-12 w-full overflow-hidden">
      {/* Search & Tabs Rebuild for Fluidity */}
              ...
              >
                <div className="flex justify-between items-start gap-4 mb-4">
                  ...
                  <div className="flex flex-wrap justify-end gap-1 shrink-0">
                    {test.isFree && <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 text-[8px] px-1.5 h-4">Free</Badge>}
                    {test.isNew && <Badge className="bg-primary/10 text-primary border-primary/20 text-[8px] px-1.5 h-4">New</Badge>}
                  </div>
                </div>
                ...
                <div className="mt-auto flex items-center justify-between gap-4">
                  ...
                  <Link href={mockUrl} className="shrink-0">
      ...
      {filteredTests.length === 0 && (
        <div className="py-20 text-center text-muted-foreground glass border-white/5 rounded-3xl w-full">
          No matching mock tests available.
        </div>
      )}
    </div>
  );
