...
export const MockTestList = ({
  ...
  const mocksQuery = useMemoFirebase(
    () => db ? query(collection(db, "mockTests"), where("examId", "==", examId), where("status", "==", "Published")) : null,
    [db, examId]
  );
  ...
  const attemptsQuery = useMemoFirebase(
    () => db && user ? query(collection(db, "users", user.uid, "mockAttempts"), where("examId", "==", examId), orderBy("completedAt", "desc")) : null,
    [db, user?.uid, examId]
  );
  ...
  const { data: activeSessions, loading: activeSessionsLoading } = useCollection<any>(activeSessionsQuery);
  ...
  // FIX: Robust Status Map merging Firestore Active Sessions + LocalStorage state checks
  const mockStatusMap = useMemo(() => {
    const map: Record<string, { latestAttempt?: any; activeSession?: any }> = {};

    // 1. Cloud data mappings
    ...
    // 2. LocalStorage structural fallback interceptor
    if (typeof window !== "undefined") {
      tests?.forEach((test) => {
        const localToken = localStorage.getItem(`test_active_${test.id}`);
        if (localToken === "true" && !map[test.id]?.activeSession) {
          map[test.id] = { ...map[test.id], activeSession: { id: test.id, isLocalFallback: true } };
        }
      });
    }

    // 3. Historic data mapping
  ...
  const totalQuestions = tests?.reduce((sum, mock) => sum + (mock.totalQuestions || 0), 0) || 0;
            ...
            {mockSubTypes.map((sub: any) => (
              <button
                ...
                className={cn(
                  ...
                  selectedSubTypeId === sub.id ? "bg-primary text-white border-primary shadow-md shadow-primary/20" : "bg-white/5 border-white/5 hover:border-primary/30"
                )}
              >
                <div className="text-sm font-bold leading-tight line-clamp-2">{sub.title}</div>
                <div className="text-[10px] opacity-70 mt-2">{tests?.filter((t) => t.subTypeId === sub.id).length} Tests</div>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="w-full grid grid-cols-1 gap-3">
        {testsLoading ? (
          ...
          <div className="w-full py-12 text-center border border-dashed border-white/10 rounded-xl">
            ...
            <p className="text-muted-foreground text-[11px] font-medium">No mocks available.</p>
          </div>
        ) : (
          filteredTests.map((test) => {
            ...
            return (
              <div key={test.id} className="w-full bg-card border border-white/5 rounded-xl p-4 hover:border-primary/30 transition-all box-border">
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    {test.isFree ? (
                      <Badge className="bg-amber-500/10 text-amber-400 border-amber-500/20 text-[8px] px-1.5 py-0.5 uppercase">Free</Badge>
                    ) : (
                      <Badge className="bg-primary/10 text-primary border-primary/20 text-[8px] px-1.5 py-0.5 uppercase">Live</Badge>
                    )}
                  </div>

                  <h3 className="text-base font-bold leading-tight break-words">{test.title}</h3>
                ...
                <div className="flex gap-2 w-full">
                  {!user ? (
                    ...
                    <>
                      ...
                      <Link href={`/exams/${categorySlug}/${examSlug}/mock/${test.id}/result/${status.latestAttempt?.id}`} className="flex-1">
                        <Button className="w-full h-10 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold flex items-center justify-center gap-1">
                          <Award className="w-3.5 h-3.5" /> Result
                        </Button>
                      </Link>
                    </>
                  ) : (
