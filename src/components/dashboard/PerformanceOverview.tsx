...
const StatCard = ({ title, value, icon: Icon, color, trend }: any) => (
  <Card className="glass border-white/10 hover:border-white/20 transition-all duration-300 group overflow-hidden relative">
    <div className={`absolute top-0 right-0 w-24 h-24 bg-${color}/10 blur-3xl -z-10`} />
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
        <StatCard title="Overall Accuracy" value="94.8%" icon={Target} color="accent" trend="2.4" />
        <StatCard title="Tests Attempted" value="42" icon={BarChart3} color="primary" trend="5" />
        ...
        <StatCard title="Time/Question" value="42s" icon={Clock} color="rose-400" />
        <StatCard title="Solved Qs" value="1,200+" icon={Zap} color="emerald-400" />
      </div>

      <div className="grid lg:grid-cols-3 gap-8">
        <Card className="lg:col-span-2 glass border-white/10 p-6">
