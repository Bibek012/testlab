...
// hydrated prop hataya — TestInterface khud manage karega
interface Props {
...
export const TestInterface = ({
  ...
  // localStorage se currentQuestionIndex restore karo
  ...
  // localStorage se language restore karo
  ...
  // ─── TIMER INIT ───
  // Sirf ek baar chalega. Existing test_end use karo, nahi hai toh naya banao.
  useEffect(() => {
  ...
  // ─── AUTOSAVE ───
  // targetEndTime null nahi hona chahiye (timer init ke baad set hoga).
  // hydrated prop ka koi guard nahi — seedha save karo.
  useEffect(() => {
    if (!targetEndTime) return; // Timer init hone ka wait karo
    ...
    const interval = setInterval(async () => {
      ...
      // Firestore save — yahi asli bug tha, hydrated check hataya
      if (user && db) {
        try {
          await setDoc(
