...
import { BookOpen, Clock, Award, ChevronRight, AlertCircle, Loader2 } from "lucide-react";
...
export default function CategoryExamsPage() {
  ...
  useEffect(() => {
    ...
    const fetchCategoryAndExams = async () => {
      ...
      try {
        ...
        // 1. Category dhundho — ID, slug, aur partial slug match sab try karo
        ...
        const categoryDoc = categoriesSnapshot.docs.find((doc) => {
          ...
          return (
            ...
            slug === cp ||
            slug === `${cp}-exams` ||       // "bihar" → "bihar-exams"
            slug.startsWith(cp) ||           // partial match
            cp === slug.replace("-exams", "") // "bihar-exams" → "bihar"
          );
        });
        ...
        // 2. Exams fetch karo — multiple field matching
        ...
        examsSnapshot.forEach((doc) => {
          ...
          const matchesCategory =
            ...
            catSlug === cp ||
            catSlug === `${cp}-exams` ||       // "bihar" matches "bihar-exams"
          ...
          if (matchesCategory) {
            fetchedExams.push({
              ...
              totalQuestions:
                data.totalQuestions || data.questionsCount || data.questionCount || 0,
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {/* ✅ pt-16 navbar height compensate karta hai, sidebar sticky top-16 se align */}
      <div className="flex flex-1 pt-16">
        <ExamSidebar currentCategory={categoryParam} />

        <main className="flex-1 p-4 md:p-8 lg:p-10 space-y-6 min-w-0">
          {/* Breadcrumb */}
